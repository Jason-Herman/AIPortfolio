...
class HW2Tests(unittest.TestCase):
    ...
    def setUp(self):
       # NOTE: add U flag
       self.romania = pickle.load(open('romania_graph.pickle', 'rUb'))
       self.romania.reset_search()
       # NOTE: add U flag
       self.atlanta = pickle.load(open('atlanta_osm.pickle', 'rUb'))
    ...
    def romania_tri_helper(self, method):
        import itertools
        keys = self.romania.node.keys()
        ...
        for goals in triplets:
            ...
            for allcombo in itertools.permutations(goals):
                ...
                if pathlen != minlen:
                    ...
                    print 'ref_len', minlen
                    print 'path',path
                    print 'refpath1:', s1, s1len
                    print 'refpath2:', s2, s2len
                    print 'refpath3:', s3, s3len
                    print allcombo
                assert pathlen == minlen
    #NOTE: MODIFIED PER PIAZZA POST
    def atlanta_bi_helper(self, method, ntest=10, assert_explored=False, **kwargs):
        # ntest = -1
        keys = list(networkx.connected_components(self.atlanta).next())
        random.shuffle(keys)
        pairs = zip(keys, keys[1:])[::2]
        # pairs = [('69383456', '551076854'),('1585252448','998987516'),('69262574','69365775')] #failing ,('','')
        # pairs = [('69262574','69365775')] - solved with larger mu
        for src, dst in pairs:
    ...
    def test_bucs_atlanta(self):
        ...
        self.atlanta_bi_helper(bidirectional_ucs, ntest=10)
    ...
    def test_bastar_nul_atlanta(self):
        # put ntest = -1 to run forever until it breaks
        self.atlanta_bi_helper(bidirectional_a_star, heuristic=null_heuristic, ntest=10)
    ...
    def test_bastar_euc_atlanta(self):
        # put ntest = -1 to run forever until it breaks
        self.atlanta_bi_helper(bidirectional_a_star, heuristic=euclidean_dist_heuristic, ntest=10)

    def test_tucs_romania(self):
        self.romania_tri_helper(tridirectional_search)
